...
def create_auction():
    ...
    try:
        minimum_value = int(request.form.get('minimum_value'))
    except:
...
def update_auction(auction_external_id):
    auction = AuctionsFinder.get_auction_by_external_id(activity_external_id)
    ...
    try:
        minimum_value = request.form.get('location')
    except:
