...
class ActivitiesFinder:
    ...
    def get_from_id(cls, id):
        return Activities.query.filter_by(id=id).first()

    @classmethod
    def get_all_from_type(cls, activity_type):
        return (
            Activities.query.filter_by(activity_type=activity_type)
    ...
    def get_all_from_type_and_event(cls, activity_type, event=None):
        ...
        return (
            Activities.query.filter_by(activity_type=activity_type, event_id=event.id)
            .order_by(Activities.day, Activities.time)
            .all()
        )
    ...
    @classmethod
    def get_all(cls):
        return Activities.query.order_by(
            Activities.day, Activities.time, Activities.activity_type_id
        ).all()
    ...
    def get_quests(cls):
        ...
        return (
            Activities.query.filter(Activities.quest == True)
            .filter(Activities.day == now)
            .order_by(Activities.day, Activities.time, Activities.activity_type_id)
            .all()
        )
    ...
    def get_from_parameters(cls, kwargs):
        try:
            return (
                Activities.query.filter_by(**kwargs)
                .order_by(Activities.day, Activities.time, Activities.activity_type_id)
                .all()
            )
        except Exception:
    ...
    def search_by_name_and_event(cls, name, event=None):
        ...
        return (
            Activities.query.filter(
                Activities.name.ilike(search) & (Activities.event_id == event.id)
            )
            .order_by(Activities.day, Activities.time, Activities.activity_type_id)
            .all()
        )
    ...
    def search_by_name(cls, name):
        ...
        return (
            Activities.query.filter(Activities.name.ilike(search))
            .order_by(Activities.day, Activities.time, Activities.activity_type_id)
            .all()
        )
    ...
    def get_company_activities_from_activity_id(cls, external_id):
        return (
            CompanyActivities.query.join(
                Activities, Activities.id == CompanyActivities.activity_id
            )
    ...
    def get_speaker_activities_from_activity_id(cls, external_id):
        return (
            SpeakerActivities.query.join(
                Activities, Activities.id == SpeakerActivities.activity_id
            )
            .filter(Activities.external_id == external_id)
            .all()
        )
    ...
    def get_activity_speakers(cls, activity):
        return (
            Speakers.query.join(
                SpeakerActivities, SpeakerActivities.speaker_id == Speakers.id
            )
            .filter(SpeakerActivities.activity_id == activity.id)
            .order_by(Speakers.name)
            .all()
        )
    ...
    def get_activity_companies(cls, activity):
        return (
            Companies.query.join(
                CompanyActivities, CompanyActivities.company_id == Companies.id
            )
            .filter(CompanyActivities.activity_id == activity.id)
    ...
    def get_activity_tags(cls, activity):
        return (
            Tags.query.join(ActivitiesTags, ActivitiesTags.tag_id == Tags.id)
            .filter(ActivitiesTags.activity_id == activity.id)
            .order_by(Tags.name)
            .all()
        )
    ...
    def get_next_company_activity(cls, company):
        ...
        return (
            Activities.query.join(
                CompanyActivities, Activities.id == CompanyActivities.activity_id
            )
            .filter(
    ...
    def get_activities_from_speaker_and_event(cls, speaker, event):
        return (
            Activities.query.filter(
                ...
                & (SpeakerActivities.activity_id == Activities.id)
                & (SpeakerActivities.speaker_id == speaker.id)
            )
            .order_by(Activities.day, Activities.time)
            .all()
        )
    ...
    def get_activities_from_event(cls, event):
        return (
            Activities.query.filter(
                ...
                & (Events.id == Activities.event_id)
                & (Activities.activity_type_id == ActivityTypes.id)
                & (ActivityTypes.show_in_schedule == True)
            )
            .order_by(Activities.day, Activities.time)
            .all()
        )
    ...
    def get_next_activity(cls):
        ...
        activity = (
            Activities.query.filter(Activities.time >= current_time)
            .order_by(Activities.time)
            .first()
        )
