...
class CompaniesFinder:
    ...
    def search_by_name(cls, name):
        search = "%{}%".format(name)
        return (
            Companies.query.filter(Companies.name.ilike(search))
    ...
    def search_by_email(cls, email):
        search = "%{}%".format(email)
        return (
            Companies.query.filter(Companies.email.ilike(search))
            .order_by(Companies.name)
            .all()
        )
    ...
    def get_companies_from_default_event(cls):
        return (
            Companies.query.filter(
                (Companies.partnership_tier != "sponsor")
                & (Companies.id == CompanyActivities.company_id)
    ...
    def get_website_company(cls, name):
        ...
        return (
            Companies.query.filter(
                Companies.name.ilike(search), Companies.show_in_website == True
            )
            .order_by(Companies.name)
            .all()
        )
